...
function Progress({ stage, mode }: { stage: InspectionStage; mode: Mode }) {
  const stages = mode === 'inspect' ? ['Téléversement de l’image…', 'Détection YOLO…', 'Analyse avec le VLM…', 'Terminé'] : ['Téléversement de l’image…', 'Détection YOLO…', 'Terminé'];
...
export default function LiveInspection() {
  ...
  const startCamera = useCallback(async () => {
    ...
    } catch (err) { setCameraError(err instanceof DOMException && err.name === 'NotAllowedError' ? 'Accès à la caméra refusé. Autorisez la caméra dans les réglages du navigateur.' : 'Caméra indisponible. Vérifiez qu’une caméra est connectée.'); }
  }, []);
  ...
  const startInspection = useCallback(async () => {
    ...
    try { while (runningRef.current && !controller.signal.aborted) {
      ...
      if (mode === 'inspect') { const detected = await detectImage(frame, controller.signal); setImageSize(detected.image_size); setLiveItems(matchTargets(detected.items, true)); }
      const completed = await run(frame, mode, controller.signal); if (!completed) break;
      if (mode === 'inspect') { setImageSize(completed.image_size); setLiveItems(matchTargets(completed.items, false)); } else setLiveItems(completed.items);
      await wait(LIVE_INTERVAL_MS, controller.signal);
    } } catch (err) { if (!(err instanceof DOMException && err.name === 'AbortError')) setCameraError('L’inspection en direct s’est arrêtée de manière inattendue.'); }
  ...
  useEffect(() => { if (!frameRef.current) return; const observer = new ResizeObserver(([entry]) => setImageSize((size) => ({ ...size, width: entry.contentRect.width, height: entry.contentRect.height }))); observer.observe(frameRef.current); return () => observer.disconnect(); }, [source]);
  ...
  return <div className="page-enter p-6 space-y-6"><header className="border-b border-border bg-card py-4 flex justify-between amber-accent-line"><div className="flex gap-3"><div className="w-1 h-6 bg-primary" /><div><h1 className="text-xl font-bold font-mono">INSPECTION EN DIRECT</h1><p className="text-xs text-muted-foreground font-mono">CAMÉRA · RAISONNEMENT QUALITÉ YOLO + VLM</p></div></div><button onClick={() => setShowSettings(!showSettings)} className="border border-border px-3 py-1.5 text-xs font-mono flex gap-2 items-center"><Settings2 className="w-3.5 h-3.5" /> PARAMÈTRES</button></header>
    {showSettings && <div className="border border-border bg-card p-4"><label className="text-xs text-muted-foreground">Mode d’inférence</label><div className="flex gap-2 mt-2">{(['detect', 'inspect'] as Mode[]).map((item) => <button key={item} onClick={() => setMode(item)} className={cn('border px-3 py-2 text-xs font-mono', mode === item ? 'border-primary text-primary' : 'border-border text-muted-foreground')}>{item === 'detect' ? 'YOLO seul' : 'YOLO + VLM'}</button>)}</div></div>}
    <div className="flex gap-2 border-b border-border"><button onClick={() => { stopInspection(); setSource('camera'); }} className={cn('px-4 py-2 text-xs font-mono border-b-2 flex gap-2', source === 'camera' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground')}><Camera className="w-3.5 h-3.5" /> CAMÉRA</button><button onClick={() => { stopInspection(); setSource('upload'); }} className={cn('px-4 py-2 text-xs font-mono border-b-2 flex gap-2', source === 'upload' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground')}><Upload className="w-3.5 h-3.5" /> TÉLÉVERSER UNE IMAGE</button></div>
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6"><main className="lg:col-span-3 space-y-4">{source === 'camera' ? <><div ref={frameRef} className="relative aspect-video bg-black border border-border overflow-hidden"><video ref={videoRef} autoPlay playsInline muted className="w-full h-full object-contain" />{!cameraActive && <div className="absolute inset-0 flex flex-col items-center justify-center gap-3"><Video className="w-10 h-10 text-muted-foreground" /><button onClick={startCamera} className="border border-primary/40 bg-primary/10 px-4 py-2 text-primary text-xs font-mono">DÉMARRER LA CAMÉRA</button></div>}{liveItems.length > 0 && frameSize.width > 0 && <BoundingBoxOverlay items={liveItems} width={frameSize.width} height={frameSize.height} sourceWidth={frameSize.width} sourceHeight={frameSize.height} />}{isLoading && <div className="absolute top-0 left-0 right-0 h-0.5 bg-primary scan-line" />}</div><canvas ref={canvasRef} className="hidden" /><div className="flex flex-wrap gap-2"><button onClick={startCamera} disabled={cameraActive} className="border border-primary/40 px-3 py-2 text-primary text-xs font-mono"><Camera className="w-3.5 h-3.5 inline mr-2" />DÉMARRER LA CAMÉRA</button><button onClick={startInspection} disabled={!cameraActive || running} className="border border-primary/40 px-3 py-2 text-primary text-xs font-mono"><ScanLine className="w-3.5 h-3.5 inline mr-2" />DÉMARRER L’INSPECTION</button><button onClick={stopInspection} disabled={!running} className="border border-border px-3 py-2 text-muted-foreground text-xs font-mono"><Square className="w-3.5 h-3.5 inline mr-2" />ARRÊTER L’INSPECTION</button><button onClick={stopCamera} disabled={!cameraActive} className="border border-red-400/40 px-3 py-2 text-red-400 text-xs font-mono"><X className="w-3.5 h-3.5 inline mr-2" />ARRÊTER LA CAMÉRA</button></div></> : <><div ref={frameRef} className="relative aspect-video border-2 border-dashed border-border overflow-hidden" onDrop={(event) => { event.preventDefault(); const file = event.dataTransfer.files[0]; if (file) void handleFile(file); }} onDragOver={(event) => event.preventDefault()}>{imageUrl ? <><img ref={imageRef} src={imageUrl} alt="Image à inspecter" className="w-full h-full object-contain" onLoad={() => imageRef.current && setImageSize({ width: imageRef.current.naturalWidth, height: imageRef.current.naturalHeight })} />{result && <BoundingBoxOverlay items={result.items} width={frameSize.width} height={frameSize.height} sourceWidth={imageSize.width} sourceHeight={imageSize.height} />}<button onClick={() => { URL.revokeObjectURL(imageUrl); setImageUrl(null); reset(); }} className="absolute top-2 right-2 border border-border bg-background p-2"><X className="w-3.5 h-3.5" /></button></> : <button onClick={() => fileRef.current?.click()} className="absolute inset-0 text-sm text-muted-foreground">Déposez une image ici ou cliquez pour la téléverser</button>}</div><input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) void handleFile(file); }} /><button onClick={() => fileRef.current?.click()} disabled={isLoading} className="w-full border border-primary/40 py-2.5 text-primary text-sm font-mono">{isLoading ? <><Loader2 className="w-4 h-4 inline mr-2 animate-spin" />{stage}</> : 'TÉLÉVERSER UNE NOUVELLE IMAGE'}</button></>}{cameraError && <div className="border border-red-400/40 bg-red-400/10 p-4 text-xs text-red-400"><AlertTriangle className="w-4 h-4 inline mr-2" />{cameraError}</div>}{error && <div className="border border-red-400/40 bg-red-400/10 p-4 text-xs text-red-400"><AlertTriangle className="w-4 h-4 inline mr-2" />{error}</div>}{isLoading && stage && <div className="border border-border bg-card p-3"><Progress stage={stage} mode={mode} /></div>}</main>
      <aside className="lg:col-span-2 space-y-4">{result ? <div className="border border-border bg-card p-4"><div className="flex justify-between"><h3 className="text-xs font-semibold text-muted-foreground uppercase">Résultat de l’inspection</h3><StatusBadge status={overall} size="md" /></div><div className="grid grid-cols-2 gap-3 mt-3 text-xs"><div>Image <strong className="font-mono">#{result.frame_id}</strong></div><div>Détections <strong className="font-mono">{result.num_detections}</strong></div><div>Taille <strong className="font-mono">{result.image_size.width}×{result.image_size.height}</strong></div><div>Date <strong className="font-mono">{formatTimestamp(result.timestamp)}</strong></div></div></div> : <div className="border border-border bg-card p-6 text-center text-sm text-muted-foreground">Démarrez une inspection pour voir les résultats.</div>}{result?.items.map((item, index) => <DetectionCard key={`${result.frame_id}-${index}`} item={item} index={index} />)}</aside></div>
  </div>;
}